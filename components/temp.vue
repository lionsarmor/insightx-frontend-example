<template>
<div id="tickerSpaceTwenty">
    <div id="app">
        <!-- Main table element -->
        <div class="row">
            <div class="col-12">
                <div style="position: relative; top: 10px; float: right; z-index:99;">
                    <input v-on:click="resetgraph()" v-model="filter" class="search" placeholder="   Search" />
                </div>
                <div class="text-left title" style="position: relative; top:10px;">Asset List</div>
            </div>
        </div>
        <div class="square2"></div>
        <div class="borderblack">
            <div style="mobileSize">
            <b-table small @sort-changed="sortingChanged" :items="finalList" :fields="fields" :filter="filter" v-on:row-clicked="rowClicked" hover1 id="twentyfourMobile" class="tabletext my-table-scroll4">
      ...
      return {'color':'green'}
            ...
            loadTable(payload){
                if (window.innerWidth < 500) {
                    ...
                    this.mobile1 = true;
                        document.getElementById('twentyfourMobile').classList.remove('my-table-scroll4')
                        document.getElementById('twentyfourMobile').classList.add('my-table-scroll4M')
                    } else {
                        document.getElementById('twentyfourMobile').classList.remove('my-table-scroll4M')
                        document.getElementById('twentyfourMobile').classList.add('my-table-scroll4')
                    }
                            let that = this
